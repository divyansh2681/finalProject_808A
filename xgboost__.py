...
class XGBoost():
    ...
    def prediction(self):
        ...
        for k in range(1, 11, 1):
            kk.append(k)
            bst = xgb.XGBRegressor(objective= "reg:linear", random_state=42, max_depth = k)
        ...
        bst = xgb.XGBRegressor(objective= "reg:linear", random_state=42, max_depth = min_r2_score_index+1)
        bst.fit(self.full_X, self.full_y)
    ...
    def curve_plotting(self, X,y): 
        ...
        """
        train_sizes, train_scores, test_scores = learning_curve(xgb.XGBRegressor(objective= "reg:linear", random_state=42, max_depth = 10), X, y, cv=10, n_jobs=-1, train_sizes=np.linspace(0.01, 1.0, 50))
